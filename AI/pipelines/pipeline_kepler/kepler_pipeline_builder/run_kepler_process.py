...
Place this file under ExoLens/kepler_pipeline_builder/run_kepler_process.py

Usage example (inside the container):
    python run_kepler_process.py --input /app/input/kepler_tce_table.csv --output /app/output
...
This script will:
 ...
 - call ExoMiner's generate_input_records.py to produce TFRecords
 - call ExoMiner's prediction entrypoint to generate a CSV of predictions into the --output folder

Notes:
 - This script expects the ExoMiner clone at ../pipeline_exominer_KEPLER_MANUAL/ExoMiner relative to the ExoLens root.
...
def find_exominer_repo(start_dir: Path) -> Path:
    # Expect project root is two levels above this script (ExoLens), so repo at ExoLens/pipeline_exominer_KEPLER_MANUAL/ExoMiner
    root = start_dir.resolve().parents[1]
    candidate = root / 'pipeline_exominer_KEPLER_MANUAL' / 'ExoMiner'
    if candidate.exists():
        return candidate
    # fallback: check sibling folder named 'pipeline_exominer_KEPLER_MANUAL'
    alt = start_dir.resolve().parents[0] / 'pipeline_exominer_KEPLER_MANUAL' / 'ExoMiner'
    if alt.exists():
        return alt
    # last resort: search for ExoMiner directory in parent directories
    for p in [start_dir.resolve(), start_dir.resolve().parent, root]:
        cand = p / 'ExoMiner'
        if cand.exists():
            return cand
    return None
...
def main():
    ...
    parser.add_argument('--exominer-path', required=False, help='Path to the local ExoMiner repository (overrides automatic search)')
    ...
    # Locate ExoMiner repository (allow override)
    if args.exominer_path:
        exominer_dir = Path(args.exominer_path).expanduser().resolve()
        if not exominer_dir.exists():
            print(f"Provided --exominer-path does not exist: {exominer_dir}")
            sys.exit(1)
    else:
        start_dir = Path(__file__).resolve()
        exominer_dir = find_exominer_repo(start_dir)
        if exominer_dir is None:
            print("Could not locate ExoMiner repository. Expected at ExoLens/pipeline_exominer_KEPLER_MANUAL/ExoMiner or similar.")
            sys.exit(1)
