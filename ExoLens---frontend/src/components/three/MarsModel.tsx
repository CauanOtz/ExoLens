...
export default function MarsModel({ distance = 12, height = '100%', controls = true, modelUrl }: ModelProps) {
  ...
  useEffect(() => {
    ...
    const local = new URL('../../assets/mars/Mars_1_6792.glb', import.meta.url).href;
    ...
    loader.load(
      ...
      (xhr) => { if (xhr && xhr.lengthComputable) console.debug(`[MarsModel] load ${Math.round((xhr.loaded / xhr.total) * 100)}%`); },
      (err) => { console.warn('[MarsModel] failed to load', err); }
    );
