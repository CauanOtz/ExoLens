...
export default function MercuryModel({ distance = 12, height = '100%', controls = true, modelUrl }: ModelProps) {
  ...
  useEffect(() => {
    ...
    const local = new URL('../../assets/mercury/Mercury_1_4878.glb', import.meta.url).href;
    ...
    loader.load(
      ...
      (xhr) => { if (xhr && xhr.lengthComputable) console.debug(`[MercuryModel] load ${Math.round((xhr.loaded / xhr.total) * 100)}%`); },
      (err) => { console.warn('[MercuryModel] failed to load', err); }
    );
