...
export default function SaturnModel({ distance = 12, height = '100%', controls = true, modelUrl }: ModelProps) {
  ...
  useEffect(() => {
    ...
    const local = new URL('../../assets/saturn/Saturn_1_120536.glb', import.meta.url).href;
    ...
    loader.load(
      ...
      (xhr) => { if (xhr && xhr.lengthComputable) console.debug(`[SaturnModel] load ${Math.round((xhr.loaded / xhr.total) * 100)}%`); },
      (err) => { console.warn('[SaturnModel] failed to load', err); }
    );
